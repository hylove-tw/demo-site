...
const AnalysisReportPage: React.FC = () => {
    ...
    // 從全域使用者中根據 record.userId 找到使用者資訊
    ...
    // 假設測試資訊從 record 中取得（也可以調整為更詳細的內容）
    const testInfo = {
    ...
    // 使用分析配置中的 renderReport 來產生報告內容
    const resultContent = config ? config.renderReport(record.result) : <p>找不到對應的分析配置</p>;
    ...
    return (
        <div className="container mx-auto p-4">
            ...
            />
            <div className="mt-4">
                <Link to="/" className="btn btn-primary">返回分析頁</Link>
            </div>
        </div>
    );
};
