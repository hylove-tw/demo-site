...
const AnalysisPage: React.FC = () => {
    ...
    const handleAnalyze = async () => {
        ...
        // 根據需求合併所有檔案資料（這裡以 concat 合併二維陣列）
                        ...
                        <tr key={record.id}>
                            ...
                            <td>
                                {record.selectedFileIds.map((fileId, index) => {
                                        ...
                                        return (
                                            // show file link by alias or filename
                                            <span key={index} className="mr-2">
                                               <Link className="badge badge-outline badge-sm" to={`/files/${file?.id}`}>{file?.alias || file?.fileName}</Link>
                                            </span>
                                        );
                                    }
                                )}
                ...
                {selectedAnalysis && (
                    <div>
                        <h2 className="text-xl font-semibold mb-2">
                            {selectedAnalysis.group ? `${selectedAnalysis.group} - ${selectedAnalysis.name}` : selectedAnalysis.name}
                        </h2>
